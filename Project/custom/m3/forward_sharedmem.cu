...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int B, const int M, const int C, const int H, const int W, const int K,const int S)
{
    ...
    */
    ...
    // load the mask tile
    for (int c = 0; c < C; c++) {
        for (int p = ty; p < K; p += TILE_WIDTH) {
            for (int q = tx; q < K; q += TILE_WIDTH) {
                mask_3d(c, p, q) = mask_4d(m, c, p, q);
            }
        }
    }
    ...
    if (h < H_out && w < W_out){
        ...
        for (int c = 0; c < C; c++) {
            for (int p = 0; p < K; p++) {
                for (int q = 0; q < K; q++) {
                    acc += mask_3d(c, p, q) * sm_3d(c, ty * S + p, tx * S + q);
                }
            }
        }
        out_4d(b, m, h, w) = acc;
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
    ...
    int H_grid = ceil((float)((H - K)/S + 1)/TILE_WIDTH);
    int Y = W_grid * H_grid;
    int shared_mem_size = C * (S * TILE_WIDTH + K - S) * (S * TILE_WIDTH + K - S) * sizeof(float) + C * K * K * sizeof(float);
    dim3 dimgrid(B, M, Y);
